...
## What's real vs modeled (the whole point)

| Layer | Status | Source |
|-------|--------|--------|
| Road distances & drive times | REAL | OpenRouteService API (live) |
| Straight-line drone distances | REAL (computed) | Haversine formula |
| All cost figures | MODELED estimates | Cited industry benchmarks, editable |
| Feasibility advice | ADVISORY | NVIDIA NIM (hosted LLM) |

No public API returns a company's real per-delivery cost — that data is internal.
This tool applies published benchmarks to real distances to produce defensible
estimates, and labels them as such. That honesty is a feature.
...
- Terrain multiplier has no industry-standard value — user-adjustable assumption
